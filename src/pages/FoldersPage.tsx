  ...
  return <div className="space-y-8 max-w-7xl mx-auto pb-16">
    ...
    <Modal isOpen={open} onClose={() => setOpen(false)} title="Create folder" subtitle="Select papers from your existing library now, or add them later." maxWidth="lg">
      <form onSubmit={create} className="space-y-5">
        <input required value={name} onChange={(event) => setName(event.target.value)} placeholder="Folder name" className="research-input w-full px-3 py-2 text-sm" />
        <textarea value={description} onChange={(event) => setDescription(event.target.value)} placeholder="Description (optional)" rows={3} className="research-textarea w-full px-3 py-2 text-sm" />
