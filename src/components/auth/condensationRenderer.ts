...
export function createCondensationRenderer(
  ...
  if (!context) {
    throw new Error('Condensation requires Canvas 2D');
  }
  ...
  const getBubbleSprite = (radius: number) => {
    ...
    if (!spriteContext) {
      throw new Error('Condensation bubble sprite unavailable');
    }
  ...
  const resize = (cssWidth: number, cssHeight: number) => {
    ...
    baseMaxDrops = Math.max(
      80,
      Math.min(240, Math.round((width * height) / (dpr * dpr) / 5200)),
    );
    ...
    for (let index = 0; index < seedCount; index += 1) {
      seedDrop(Math.random() * 3);
    }
  };

  const step = (now: number, motionScale = 1) => {
    if (drops.length < maxDrops && Math.random() < 0.34 * motionScale) {
      seedDrop();
    }
